...
from app.services.buscar_bibliotecarios import buscar_bibliotecarios
from app.services.buscar_biliotecario_por_id import buscar_bibliotecario_por_id
from app.services.cadastrar_bibliotecario import cadastrar_bibliotecario
from app.services.atualizar_bibliotecario import atualizar_bibliotecario
from app.services.deletar_bibliotecario import deletar_bibliotecario
...
def listar_bibliotecario_por_id(bibliotecario_id):
    ...
    if not bibliotecario:
        return jsonify({"erro": "Bibliotecario não encontrado"}), 404
...
def remover_bibliotecario(bibliotecario_id):
    ...
    if not removido:
        return jsonify({"erro": "Bibliotecario não encontrado"}), 404
    return jsonify({"mensagem": "Bibliotecario removido com sucesso"}), 200